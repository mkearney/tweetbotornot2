...
1. **On a machine with a local version of R**, send API request via {rtweet}
   ...
   ```
1. On the same machine, save the token as an `.rds` file
   ```{r, eval=FALSE}
   saveRDS(rtweet::get_token(), "rtweet_token.rds")
   ```
1. Upload the saved token file to the **cloud/server**
...
To avoid having to specify the token each time, you can also set the path to 
the token as an R environment variable, i.e.,
...
To set the R environment variable for current **and** future sessions, you can 
also try `tfse::set_renv()`.
...
token rate limit before and after the `predict_bot()` call.
...
For the purpose of examples, I will use a list of accounts associated with
candidates in the 2020 U.S. Democratic presidential primary.
